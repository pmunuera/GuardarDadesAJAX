...
public class ControllerFormulari implements Initializable {
    @FXML
    private TextField nom,cognoms,correu_electronic,telefon,direccio,ciutat;

    @FXML
    private Label title,errorNom,errorCognoms,errorCorreu,errorTelefon,errorDireccio,errorCiutat,textAfegit;
    @FXML
    private Button afegir,modificar;
    @FXML
    ...
    public void afegir(){
        ...
        UtilsHTTP.sendPOST(Main.protocol + "://" + Main.host + ":" + Main.port + "/dades", obj.toString(), (response) -> {
            ...
            enterCallback(response,"Afefir");
            hideLoading();
        });

    }

    public void enterCallback(String response,String accio){
